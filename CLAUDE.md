...
- **Squadre degli allenamenti = dati nominativi:** il JSON `TrainingSession.teams` contiene nome, ruolo e genere di ogni atleta. `/api/sessions`, `/api/sessions/[id]`, `/api/teams/[sessionId]`, la home e `/allenamenti` lo passano solo ai tesserati; per gli altri `teams` è `null`
