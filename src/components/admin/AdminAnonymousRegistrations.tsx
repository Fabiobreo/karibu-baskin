...
export default function AdminAnonymousRegistrations({
  registrations,
  bare,
}: {
  ...
  function handleDeleteConfirm() {
    ...
    startTransition(async () => {
      await fetch(`/api/registrations?name=${encodeURIComponent(nameToDelete)}`, {
        method: "DELETE",
      });
      setGroups((prev) =>
        prev.filter((g) => g.name.toLowerCase().trim() !== nameToDelete.toLowerCase().trim())
      );
      router.refresh();
    });
  }
                ...
                <TableCell align="right" sx={{ pr: 0 }}>
                  <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 0.5 }}>
                    ...
                    <Tooltip title="Elimina tutte le iscrizioni di questo nome">
                      <IconButton
                        size="small"
                        aria-label="Elimina tutte le iscrizioni di questo nome"
                        color="error"
                        disabled={isPending}
          ...
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            ...
            <Box>
              ...
              <Select
                ...
                onChange={(e) =>
                  setEditState((s) => ({ ...s, role: e.target.value, roleVariant: "" }))
                }
              >
                <MenuItem value="">
                  <em>Non impostato</em>
      ...
      <Dialog open={!!confirmGroup} onClose={() => setConfirmGroup(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <DialogContentText>
            ...
            <strong>{confirmGroup?.name}</strong> da tutti gli allenamenti. L&apos;operazione non è
            reversibile.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
