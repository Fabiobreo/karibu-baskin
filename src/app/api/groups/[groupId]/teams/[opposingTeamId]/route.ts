...
type Params = { params: Promise<{ groupId: string; opposingTeamId: string }> };
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  const { groupId, opposingTeamId } = await params;
  ...
  try {
    await prisma.groupTeam.delete({
      where: { groupId_opposingTeamId: { groupId: g.id, opposingTeamId } },
    });
  } catch (err) {
